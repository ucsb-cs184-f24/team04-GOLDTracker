...
//lines 12-15 taken from https://github.com/firebase/functions-samples/blob/main/Node-1st-gen/authorized-https-endpoint/functions/index.js
...
exports.search = required.https.onRequest(async (request, response) => {
    //lines 21-31 taken from https://github.com/firebase/functions-samples/blob/main/Node-1st-gen/authorized-https-endpoint/functions/index.js
    ...
    let sendRequest = new Request(url, {headers:headers} );
    let gotResponse = await fetch(sendRequest).then(response => response.json());
...
exports.poll = required.https.onRequest(async (request, response) => {
    //lines 45-56 taken from https://github.com/firebase/functions-samples/blob/main/Node-1st-gen/authorized-https-endpoint/functions/index.js
    if(!request.headers.authorization) {
        response.status(401).send("Unauthorized");
        return;
    }
