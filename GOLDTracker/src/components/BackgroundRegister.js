...
async function prepareNotification(availableCourses){
    if (availableCourses.length > 0 && (await getPresentedNotificationsAsync()).length === 0) {
...
export async function runBackgroundNotificationSequence(){
    ...
    await prepareNotification(availableCourses);
